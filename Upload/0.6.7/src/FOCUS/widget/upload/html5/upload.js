;void function(window, document){
	...
	FOCUS.widget.Upload_HTML5 = function(cfg){
		this.constructor = arguments.callee;
		this.mode = FOCUS.widget.Upload.MODE.HTML5;
	...
	FOCUS.widget.Upload_HTML5.prototype = new FOCUS.widget.UploadBase();
	FOCUS.util.extend(true, FOCUS.widget.Upload_HTML5.prototype, {
		init: function(){
			var _this = this;
			...
			this._.start = function(){
				...
				if(file){
					if(!_this.cfg.uploadURL || FOCUS.util.type(_this.cfg.uploadURL) !== 'string'){
						_this.cfg.events.uploadError.call(_this, file, FOCUS.widget.Upload.UPLOAD_ERROR.MISSING_UPLOAD_URL, 'missing upload url.');
			...
			this._.error = function(e, code, msg){
				if(!_this._.cancel){
					...
					_this.cfg.events.uploadError.call(_this, file, code || FOCUS.widget.Upload.UPLOAD_ERROR.UPLOAD_FAILED, msg || '');
					_this._.uploadComplete();
				}
			};
			...
			this._.post = function(file){
				//setTimeout(function(){
					...
					if(FOCUS.util.feature_upload.html5.FormData){
			...
			this._.xhrComplete = function(e){
				if(this.readyState === 4){
					var ERR = FOCUS.widget.Upload.UPLOAD_ERROR;
					switch(this.status){
						case 200: {
						...
						case 404:
						case 500: {
							_this._.error.call(this, e, ERR.HTTP_ERROR, this.status);
						}; break;
						default:;
					}
				}
			...
			this._.__queued = function(files, callback){
				...
				if(_this.cfg.queueLimit !== -1 && (selected.length + _this._.queue.length) > _this.cfg.queueLimit){
					for(var i = 0, len = selected.length; i < len; i++){
						_this.cfg.events.queueError.call(_this, selected[i], FOCUS.widget.Upload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED, _this.cfg.queueLimit);
					}
				}else{ // other validations
					for(var i = 0, len = selected.length; i < len; i++){
						file = new FOCUS.widget.Upload.File(selected[i]);
						
						file.status = FOCUS.widget.Upload.FILE_STATUS.ERROR;

						if(!FOCUS.util.mimeValidation(_this.cfg.fileTypes, file.type)){
							_this.cfg.events.queueError.call(_this, file, FOCUS.widget.Upload.QUEUE_ERROR.INVALID_FILETYPE, 'File is not an allowed file type.');
							continue;
						}
						
						//zero size file
						if((file.fileSize || file.size) === 0){
							_this.cfg.events.queueError.call(_this, file, FOCUS.widget.Upload.QUEUE_ERROR.ZERO_BYTE_FILE, 'File is zero bytes and cannot be uploaded.');
							continue;
						}
						
						var sizeLimit = FOCUS.util.sizeConvert.unit2Bytes(_this.cfg.sizeLimit);
						//exceed size limit
						if(_this.cfg.sizeLimit !== -1 && (file.fileSize || file.size) > sizeLimit){
							_this.cfg.events.queueError.call(_this, file, FOCUS.widget.Upload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT, 'File size exceeds allowed limit.');
							continue;
						}
				...
				if(isFromDialog){
					...
					//TODO
					FOCUS.util.type(callback) === 'function' && callback.call(_this, selected.length, queued.length, total.length);
				}
			...
			this.elems.selector.multiple = FOCUS.util.feature_upload.html5.multiple && this.cfg.multiple;
			this.elems.holder.accept = this.cfg.fileTypes;
			
			var place = document.querySelectorAll(this.cfg.placeholder);
			if(place.length){
				place = place[0];
		...
		startUpload: function(){
			...
			FOCUS.util.event.bind(this.elems.poster.upload, 'progress', this._.progress);
			FOCUS.util.event.bind(this.elems.poster.upload, 'error', this._.error);
			FOCUS.util.event.bind(this.elems.poster.upload, 'abort', this._.abort);
			FOCUS.util.event.bind(this.elems.poster, 'readystatechange', this._.xhrComplete);
		...
		setFileTypes: function(mimeTypes){
			this.cfg.fileTypes = this.elems.selector.accept = FOCUS.util.convert2Mimetype(mimeTypes);
		},
