...
IF NOT EXIST release\swf MD release\swf
FOR /F "eol=# tokens=* delims= " %%i IN (package\swf.js) DO (
	COPY %%i release\swf\
)

rem copy css
IF NOT EXIST release\css MD release\css
FOR /F "eol=# tokens=* delims= " %%i IN (package\css.js) DO (
	COPY %%i release\css\
)

rem copy libs
IF NOT EXIST release\lib MD release\lib
FOR /F "eol=# tokens=* delims= " %%i IN (package\lib.js) DO (
	XCOPY %%i release\lib\ /E /Y
)

rem adapter
FOR /F "eol=# tokens=* delims= " %%i IN (package\adapter.js) DO (
	COPY %%i release\
	java -jar package\compiler.jar --js release\adapter.js --js_output_file release\adapter.min.js
)
